// Mirrors lib/server/whitelist/client.ts (kept as plain JS so the worker can
// run without a TypeScript build step, same convention as ksef-client.mjs).
//
...
export function interpretWhitelistResponse(ok, body) {
  ...
  if (!ok && body && typeof body === "object" && "code" in body) {
    return { status: "invalid_input", requestId: null, message: body.message || body.code || "Nieprawidłowe dane zapytania." };
  }
  return { status: "check_failed", requestId: null, message: null };
}

export async function checkNipBankAccount(nip, bankAccount, date) {
