...
import { validateLogoUpload } from "@/lib/server/file-validation";
...
export async function POST(request: NextRequest) {
  ...
  if (!(file instanceof File) || file.size < 1 || file.size > MAX_LOGO_SIZE) {
    return NextResponse.json({ error: "Dodaj logo PNG, JPG lub WebP do 2 MB." }, { status: 400 });
  }
  const bytes = Buffer.from(await file.arrayBuffer());
  const validatedFile = validateLogoUpload(bytes, file.type);
  if (!validatedFile) {
    return NextResponse.json({ error: "Zawartość pliku nie odpowiada formatowi PNG, JPG lub WebP." }, { status: 400 });
  }
  const key = `${session.tenantId}/branding/${randomUUID()}${validatedFile.extension}`;
  ...
  try {
    ...
    await withTenantTransaction(session.tenantId, session.userId, async (client) => {
      await client.query(`update tenants set settings = jsonb_set(coalesce(settings, '{}'::jsonb), '{branding}', coalesce(settings->'branding', '{}'::jsonb) || $1::jsonb, true), updated_at = now() where id = $2`, [JSON.stringify({ logoKey: key, logoMime: validatedFile.mimeType }), session.tenantId]);
      await client.query("insert into audit_log (tenant_id, actor_user_id, action, entity_type, entity_id, after_data) values ($1, $2, 'tenant.logo_updated', 'tenant', $1, jsonb_build_object('mime_type', $3::text, 'size', $4::bigint))", [session.tenantId, session.userId, validatedFile.mimeType, file.size]);
    });
